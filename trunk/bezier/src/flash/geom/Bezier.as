 ...
 * Примеры решения других практических задач вынесены в отдельный пакет howtodo.
 * 
 * 			III. КОНЦЕПЦИИ 
 * 1. Классы Bezier и Line реализованы схожим образом и подавляющее большинство 
 * их методов имеют аналогичный, либо схожий синтаксис.
 ...
 *  
 * 2. Точка на Line и Bezier одномерна и характеризуется time-итератором. 
 * Возможно, это несколько непривычно, что при вычислении точки на кривой 
 * возвращается не привычный всем объект класса Point, а time-итератор, 
 * являющийся Number. Однако такая реализация позволяет при расчетах избежать 
 * избыточных конвертаций при последующих расчетах.
 * При необходимости перевести точку в двумерную, используйте метод getPoint().  
 * 
 * 3. Объекты Bezier и Line могут быть сегментами или бесконечными.
 * Оба класса имеют свойство isSegment, по умолчанию true, которое указывает
 * на то, в каких пределах находить значения.
 * [TODO]
 * 
 **/ 
